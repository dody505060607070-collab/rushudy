...
function ReservationsPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`تم حجز العقار لمدة ${durationOptions.find((item) => item.value === form.duration_hours)?.label ?? "24 ساعة"}`);
    },
  ...
  const update = useMutation({
    ...
    onSuccess: ({ action, contractId }) => {
      refresh();
      toast.success(action === "extend" ? "تم تمديد الحجز 24 ساعة" : action === "cancel" ? "تم إلغاء الحجز" : "تم إنشاء مسودة العقد من الحجز");
    ...
    <>
      <PageHero
        ...
        stats={[
          ...
          { value: String(rows.filter((row) => row.status === "converted").length), label: "تحولت لعقود" },
          { value: String(rows.filter((row) => row.status === "expired").length), label: "حجز منتهي" },
        ]}
      />
      ...
      {query.isLoading ? (
        ...
        <DataTable<Row>
          ...
          emptyState={<EmptyState text="لا توجد حجوزات" hint="أنشئ حجزًا جديدًا ليظهر هنا مع حالته ومدته." />}
          columns={[
            { header: "العقار", value: (row) => row.properties?.name, cell: (row) => row.properties?.name ?? "—", className: "font-semibold" },
            { header: "الكود", value: (row) => row.properties?.code, cell: (row) => row.properties?.code ?? "—" },
            { header: "الموظف", value: (row) => row.employee?.full_name, cell: (row) => row.employee?.full_name ?? "—" },
            { header: "العميل", value: (row) => row.contact?.full_name, cell: (row) => row.contact?.full_name ?? "—" },
            { header: "بواسطة", value: (row) => row.creator?.full_name, cell: (row) => row.creator?.full_name ?? "—" },
            { header: "أُنشئ", value: (row) => row.created_at, cell: (row) => formatDate(row.created_at) },
            { header: "ينتهي", value: (row) => row.ends_at, cell: (row) => formatDate(row.ends_at) },
              ...
              cell: (row) =>
                canBook && ["hold", "active"].includes(row.status) ? (
                  <span className="flex flex-wrap gap-2">
                    <Button size="sm" variant="outline" onClick={() => update.mutate({ row, action: "extend" })} disabled={update.isPending}>
                      <Clock3 />تمديد 24س
                    </Button>
                    <Button size="sm" onClick={() => update.mutate({ row, action: "approve" })} disabled={update.isPending}>
                      <CheckCircle2 />تحويل لعقد
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => update.mutate({ row, action: "cancel" })} disabled={update.isPending}>
                      <XCircle />إلغاء
                    </Button>
                  </span>
                ) : "—",
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="العقار" required className="sm:col-span-2">
            <select className={inputClass} value={form.property_id} onChange={(event) => setForm((current) => ({ ...current, property_id: event.target.value }))}>
              <option value="">اختر العقار</option>
              {options.data?.properties.map((property) => <option key={property.id} value={property.id}>{property.code} — {property.name}</option>)}
            </select>
          </Field>
          <Field label="الموظف" required>
            <select className={inputClass} value={form.employee_id} onChange={(event) => setForm((current) => ({ ...current, employee_id: event.target.value }))}>
              <option value="">اختر الموظف</option>
              {options.data?.staff.map((profile) => <option key={profile.id} value={profile.id}>{profile.full_name}</option>)}
            </select>
          </Field>
          <Field label="مدة الحجز" required>
            <select className={inputClass} value={form.duration_hours} onChange={(event) => setForm((current) => ({ ...current, duration_hours: Number(event.target.value) as DurationHours }))}>
              {durationOptions.map((duration) => <option key={duration.value} value={duration.value}>{duration.label}</option>)}
            </select>
          </Field>
          <Field label="العميل" className="sm:col-span-2">
            <select className={inputClass} value={form.contact_id} onChange={(event) => setForm((current) => ({ ...current, contact_id: event.target.value }))}>
              <option value="">بدون عميل</option>
              {options.data?.contacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.full_name}</option>)}
            </select>
          </Field>
