...
/** ينشئ مسودة عقار مرتبطة بالطلب بدون نشرها، ليكملها الموظف أولًا. */
export const prepareListingRequestProperty = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (req.property_id) return { propertyId: req.property_id as string };

    const { data: created, error: createError } = await supabase
      .from("properties")
      .insert({
        code: `LR-${Date.now().toString(36).toUpperCase()}`,
    ...
    const { error: updateError } = await supabase
      .from("listing_requests")
      .update({ property_id: created.id, status: "in_review", updated_at: new Date().toISOString() })
      .eq("id", data.requestId);
    if (updateError) throw new Error(updateError.message);
    return { propertyId: created.id as string };
  });

/** اعتماد طلب عرض عقار بعد إكمال بياناته: نشر العقار + إشعار المالك عبر واتساب. */
export const approveListingRequest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!propertyId) throw new Error("افتح العقار في النظام وأكمل بياناته أولًا");
